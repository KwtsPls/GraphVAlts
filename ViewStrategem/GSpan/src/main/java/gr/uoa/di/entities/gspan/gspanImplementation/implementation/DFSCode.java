 ...
 * @author Chao Cheng
 */
class DFSCode implements Serializable{
     ...
     */
    DFSCode copy() throws IOException, ClassNotFoundException {
    ...
    ExtendedEdge getAt(int i) {
        return eeL.get(i);
    }

    boolean onRightMostPath(int v) {
        return rightMostPath.contains(v);
    }

    boolean containEdge(int v1, int v2) {
        for (ExtendedEdge ee : eeL) {
            int eeV1 = ee.getV1();
            int eeV2 = ee.getV2();
