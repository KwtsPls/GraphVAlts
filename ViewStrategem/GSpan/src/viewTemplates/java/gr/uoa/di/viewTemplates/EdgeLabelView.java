...
public class EdgeLabelView extends View {
    ...
    List<String> replacedEdge;
    double benefit;
    double cost;

    public EdgeLabelView(Translator translator, Connector connector, PatternVS queryPattern, int viewName){
    ...
    public EdgeLabelView(Translator translator, Connector connector, PatternVS queryPattern, int viewName, String edge){
        super(translator,connector,queryPattern,viewName);
        List<String> triple = CypherQuery.createEdge(edge);
        if(triple.size()!=3) this.replacedEdge=null;
        else this.replacedEdge=triple;
        this.benefit=-1.0;
    }

    public EdgeLabelView(Translator translator, Connector connector, PatternVS queryPattern, int viewName, String edge, double cost){
        super(translator,connector,queryPattern,viewName);
        List<String> triple = CypherQuery.createEdge(edge);
        if(triple.size()!=3) this.replacedEdge=null;
        else this.replacedEdge=triple;
    ...
    public double getBenefit() {
        if(benefit==-1.0) {
            Pair<Double, List<String>> p = connector.edgeLabelBenefit(this.translator.convert(this.queryPattern), this.viewName);
            if(p!=null) {
                this.replacedEdge = p.getRight();
    ...
    public Double materialize() {
        if(this.replacedEdge!=null) {
            Query q = translator.convert(queryPattern);
            System.out.println(q.edgeLabelMaterialization(this.replacedEdge, this.viewName));
            Long r = connector.write(q.edgeLabelMaterialization(this.replacedEdge, this.viewName));
    ...
    public Double materializeThroughFile(String importPath){
        if(this.replacedEdge!=null){
            ...
            try {
                ...
                view = new BufferedWriter(new FileWriter(systemPath+"edge_View_"+viewName+".csv"));
                view.write("start|end\n");

                //Get the headers
                String line = rels.readLine();
                String headers[] = line.split("\\|",-1);
                String view_edge = this.replacedEdge.get(0).replaceAll("`","");

                line = rels.readLine();
                while (line != null) {
                    String[] columns = line.split("\\|",-1);
                    if(columns[2].equals(view_edge)){
                        if(!edges.contains(columns[0]+"|"+columns[1])) {
                            view.write(columns[0] + "|" + columns[1] + "\n");
                            edges.add(columns[0]+"|"+columns[1]);
                        }
                    }
                    line = rels.readLine();
                }
            ...
            relsQuery.append("LOAD CSV WITH HEADERS FROM 'file:///").append("edge_View_"+viewName+".csv' AS line\n");
    ...
    @Override
    public void delete() {
        connector.delete(translator.convert(this.queryPattern).edgeLabelDeletion(this.viewName));
    }
    ...
    public Query rewrite(PatternVS pattern) {
        ...
        if(!contains.isEmpty()){
            for (ContainmentMapping<NodeVS, TripleVS, TrieMetadata<View>> containmentMapping : contains) {
                ...
                String replacedEdgeString = "(" + this.replacedEdge.get(1) + "," + this.replacedEdge.get(0) + "," + this.replacedEdge.get(2) + ")";
                String viewEdgeString = "(?" + this.replacedEdge.get(1) + ",View_" + this.viewName + ",?" + this.replacedEdge.get(2) + ")";
        ...
        //Special case of cycle containment
        if(translator.convert(pattern).isCyclic()){
            if(this.replacedEdge!=null && this.replacedEdge.size()==3) {
                String query = pattern.toFullString().replaceAll(this.replacedEdge.get(0).replaceAll("`",""), "View_" + viewName);
                return this.translator.convert(query);
            }
        }
    ...
    public double executeThroughView(PatternVS query) {
        ...
        System.out.println("EDGE_"+viewName+" -> " + pair.getRight().getLeft());
